<div class="min-h-screen bg-gray-100 p-6">
    <div class="bg-white p-6 rounded shadow w-full max-w-5xl mx-auto">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-4">
            <button wire:click="setFilter('all')" class="cursor-pointer px-3 py-1 rounded text-sm font-medium border border-gray-300 mr-2" :class="{'bg-blue-100 text-blue-700': @js($filter) === 'all'}">All</button>
            <button wire:click="setFilter('images')" class="cursor-pointer px-3 py-1 rounded text-sm font-medium border border-gray-300 mr-2" :class="{'bg-blue-100 text-blue-700': @js($filter) === 'images'}">Images</button>
            <button wire:click="setFilter('docs')" class="cursor-pointer px-3 py-1 rounded text-sm font-medium border border-gray-300" :class="{'bg-blue-100 text-blue-700': @js($filter) === 'docs'}">Docs</button>
            <form wire:submit.prevent="createFolder" class="flex items-center ml-auto gap-2">
                ...
                <button type="submit" class="bg-green-500 text-white px-3 py-1 rounded text-sm disabled:cursor-not-allowed disabled:opacity-50" :disabled="!$wire.newFolderName">Create Folder</button>
            </form>
        </div>
        <div class="flex flex-col md:flex-row gap-4 mb-4">
            <div class="flex-1">
                <div class="border-2 border-dashed border-gray-300 rounded p-4 py-8 flex flex-col items-center justify-center cursor-pointer bg-gray-50 hover:bg-gray-100"
                ...
                >
                    <label class="cursor-pointer w-full flex flex-col items-center">
                        <span class="text-gray-500">Drag & drop files here or click to browse</span>
        ...
        <div class="flex flex-col md:flex-row gap-8">
            <div class="flex-1">
                ...
                <ul class="mb-4">
                    @if ($currentPath !== '/')
                        <li>
                            <button wire:click="goToFolder('{{ dirname($currentPath) }}')" class="text-blue-500 hover:underline cursor-pointer">.. (Up)</button>
                        </li>
                    @endif
                    ...
                    @forelse ($files as $file)
                        <li class="flex items-center justify-between group py-1">
                            <div class="flex items-center">
                                ...
                                @else
                                    <svg class="w-5 h-5 mr-2 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-file-text-icon lucide-file-text"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/></svg>
                                @endif
