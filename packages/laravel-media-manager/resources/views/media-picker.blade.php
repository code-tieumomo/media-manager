<div>
    <div class="border-2 border-dashed border-gray-300 rounded p-6 text-center cursor-pointer hover:bg-gray-50 transition relative" wire:click="open" wire:ignore.self>
        @if(is_array($selectedImage) && count($selectedImage) > 0)
            <div class="flex flex-wrap gap-2 justify-center">
                @foreach($selectedImage as $imgUrl)
                    <img src="{{ $imgUrl }}" class="w-16 h-16 object-cover rounded border" />
                @endforeach
            </div>
        @else
            <div class="text-gray-400 flex flex-col items-center justify-center">
                <svg class="w-10 h-10 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4a1 1 0 011-1h8a1 1 0 011 1v12m-4 4h-4a1 1 0 01-1-1v-4h6v4a1 1 0 01-1 1z" /></svg>
                <span>Click to select or upload image(s)</span>
            </div>
        @endif
    </div>
